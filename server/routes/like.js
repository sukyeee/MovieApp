...
router.post('/likeNumber', (req, res) => { //front에서 post로 요청해서, post로 응답해줌. front에서 보낸 variable은 req로 받음
    ...
    Like.find({ "postId": req.body.postId, "commentId":req.body.commentId }) //DB안에 req.body.movieId와 같은게 있으면 ok
...
router.post('/liked', (req, res) => { //front에서 post로 요청해서, post로 응답해줌. front에서 보낸 variable은 req로 받음
    ...
    Like.find({ "postId": req.body.postId, "userFrom":req.body.userFrom ,"commentId":req.body.commentId })
        .exec((err, info) => {
            if (err) return res.status(400).send(err)
...
router.post('/removeFromLike', (req, res) => {
    ...
    Like.findOneAndDelete({ "postId": req.body.postId, "userFrom": req.body.userFrom,"commentId":req.body.commentId  })
        .exec((err, doc) => {
            if (err) return res.status(400).send(err)
